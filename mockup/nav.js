 ...
 *
 * 各モック画面（login.html / chat.html / upload.html）共通のヘッダーメニューを
 * #app-header 要素に描画する。ログイン状態・ロールは実サーバーが存在しないため
 * localStorage で画面間共有する（本番実装ではlocal-user-authenticationが
 * current_userを解決する想定 = NavLinkPolicy/HeaderMenuViewの設計を模す）。
 */
(function () {
  'use strict';
  ...
  // ===== ログアウト操作（本来はlocal-user-authenticationのPOST /logoutに委譲） =====
  function mockLogout() {
